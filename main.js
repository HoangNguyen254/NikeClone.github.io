...
for (var i = 0; i < purchase.length; i++) {
    purchase[i].onclick = function () {
        ...
        var convertPrice = convertToNumber(`${shoesName[1].innerHTML}`);
        ...
        if (checkDupli == false) {
            ...
            // console.log(objectShoes.totalPrice)
            objectShoes.totalPrice = convertToString(convertPrice * objectShoes.amount);
...
function renderListShoes(arr) {
    var content = arr.reduce(function (glcontent, value, index) {
        glcontent += `<li class="itemShoesSide">
        <div class="row">
            ...
            <div class="col-9">
                ...
                <div class="d-flex itemTitle justify-content-between">
                    <span>Số lượng</span>
                    <span class="priceMain">${value.totalPrice}</span>
                </div>
...
function ascend(i) {
    var priceItem = document.querySelectorAll(".itemShoesSide  .priceMain")[Number(i)];
    ...
    shoesArr.forEach(function (item, index) {
        if (index == i) {
            ...
            var oneItem = (item.amount) * bill;
            oneItemConvert = convertToString(oneItem);
            // item.price = oneItemConvert + "<u>đ</u>";
            // item.totalPrice = 
            item.totalPrice = oneItemConvert + "<u>đ</u>"
            total += bill;
        }
    });
    countProducts++;
...
for (var i = 0; i < cardSelect.length; i++) {
    cardSelect[i].onclick = function () {
        ...
        document.querySelector(`#${grandParent} .active`).className = "card-select";
        var childrenImg = this.children[0].src;
        // console.log(childrenImg)
        this.className += " active";
        document.querySelector(`#${grandParent} > img`).src = childrenImg;
    }
}
